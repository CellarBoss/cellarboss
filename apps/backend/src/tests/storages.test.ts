...
import { createTestApp, createTestAppWithAuth, runMigrations, createTestLocation } from './setup.js';
    ...
    describe('POST /storage', () => {
      ...
      it('creates a storage with valid data', async () => {
        const res = await app.request('/storage', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: 'Rack A', locationId: testLocationId, parent: null }),
        });
    ...
    describe('GET /storage/:id', () => {
      ...
      it('returns storage by id', async () => {
        const createRes = await app.request('/storage', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: 'Shelf 1', locationId: null, parent: null }),
        });
    ...
    describe('PUT /storage/:id', () => {
      it('updates a storage', async () => {
        const createRes = await app.request('/storage', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: 'Bin A1', locationId: null, parent: null }),
        });
        const created = await createRes.json();
    ...
    describe('DELETE /storage/:id', () => {
      it('deletes a storage', async () => {
        const createRes = await app.request('/storage', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: 'Box 1', locationId: null, parent: null }),
        });
        const created = await createRes.json();
