...
export default function RegionsPage() {
  ...
  async function handleBulkDelete(rows: Region[]): Promise<void> {
    for (const row of rows) {
      ...
      if (!result.ok)
        throw new Error("Error deleting region: " + result.error.message);
    }
    queryClient.invalidateQueries({ queryKey: ["regions"] });
  }
  ...
  async function handleBulkEdit(
    ...
    for (const row of rows) {
      ...
      if (!result.ok)
        throw new Error("Error updating region: " + result.error.message);
    }
    queryClient.invalidateQueries({ queryKey: ["regions"] });
  }
    ...
    },
    {
      ...
      cell: ({ row }: { row: { original: Region } }) => {
        const country = countryList.filter(
          (country) => country.id === row.original.countryId,
        )[0];
        return <a href={"/countries/" + country.id}>{country.name}</a>;
      },
    },
    {
      accessorKey: "options",
  ...
  return (
    <section>
      ...
      <DataTable<Region>
        ...
        buttons={[
          <AddButton
            onClick={async () => router.push(`/regions/new`)}
            subject="Region"
            key="add"
          />,
        ]}
